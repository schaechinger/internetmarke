...
class Address {
  ...
  constructor({ street, houseNo, zip, city, country, additional = null }) {
    ...
    // convet houseNo and zip if necessary
    if ('number' === typeof this._houseNo) {
      this._houseNo += '';
    }
    if ('number' === typeof this._zip) {
      this._zip += '';
    }
  ...
  getData() {
    ...
    if (this._additional) {
      address.additional = this._additional.substr(0, 50);
    }

    let data = {};

    if (this.isNamed()) {
      data = this._name.getName();
    }
