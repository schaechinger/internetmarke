...
export const formatAddress = (data: NamedAddress): NamedAddress => {
  // address
  data.address.street = data.address.street.substr(0, 50);
  data.address.houseNo = data.address.houseNo.substr(0, 10);
  if (data.address.additional) {
    data.address.additional = data.address.additional.substr(0, 50);
  }
  data.address.city = data.address.city.substr(0, 35);
  data.address.zip = data.address.zip.substr(0, 10);
  if (!data.address.country) {
    data.address.country = CountryCode.DEU;
  }

  // name
  let personName: PersonName | null = (data as PersonAddress).name.personName;

  // company name
  if ((data as CompanyAddress).name.companyName) {
    const companyAddress = data as CompanyAddress;

    companyAddress.name.companyName.company =
      companyAddress.name.companyName.company.substr(0, 50);

    if (companyAddress.name.companyName.personName) {
      personName = companyAddress.name.companyName.personName;
    }
  }

  // person name
  if (personName) {
    personName.firstname = personName.firstname.substr(0, 35);
    personName.lastname = personName.lastname.substr(0, 35);
    if (personName.salutation) {
      personName.salutation = personName.salutation.substr(0, 10);
    }
    if (personName.title) {
      personName.title = personName.title.substr(0, 10);
    }
  }
